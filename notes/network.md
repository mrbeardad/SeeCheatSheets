...
* 定时器：
    > 不是为每个数据包都维护一个定时器（开销大），而是只维护一个定时器，超时时重传为确认接收的最早的报文；  
    ...
    * 从应用层接收到待发送的数据：
        ...
        * 设置并更新发送序号`Seq`
    * 收到ACK
        * 若ACK值大于接收序号`Base`则更新`Base`
        * 若存在未确认的报文段(Base小于等于上次发送的报文段序号)则重启定时器，并重新计算时间间隔
        * 若连续收到3次冗余ACK则重传序号为`Base`的报文段（快速重传）
    * 定时器超时
        * 若序号为`Base`的报文段未被确认则重传它，并将超时间隔加倍
        * 重启定时器

* 接收方回传ACK事件
    * 期望序号按序到达：延迟ACK，对下一个报文最多等待500ms
    * 期望序号按序到达，另一报文等待ACK：立即发送累积ACK
    * 检测出间隔：立即发送冗余ACK
