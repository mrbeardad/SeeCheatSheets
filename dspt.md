...
> 好吧我只是为了顺口才这么写，单一职责和迪米特不是这意思

- 依赖倒置原则：面向接口编程从而解耦调用方与实现方
- 接口分离原则：拆分接口使其粒度足够细可提高整体灵活性
- 里氏替换原则：**is-a**关系可通过**继承抽象**来实现接口功能（实现、泛化）
- 单一职责原则：**has-a**关系可通过**组合对象**来实现接口功能（组合、聚合）
- 迪米特法则：**use-a**关系可通过**组合对象**或**调用对象**来实现接口功能（关联、依赖）
- 开放关闭原则：最终目的为实现一个高可复用的系统（模块的可替换性与可扩展性）
