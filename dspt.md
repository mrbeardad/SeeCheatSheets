...
- 依赖倒置原则：高层模块与底层模块都应面向稳定的接口编程，从而封装实现
- 接口分离原则：拆分接口使其粒度足够细，从而避免客户依赖他们不用的方法
- 单一职责原则：通过组合对象来实现接口功能，适用于 has-a 关系
- 里氏替换原则：通过继承抽象来实现接口功能，适用于 is-a 关系
- 开放关闭原则：最终目的为实现一个高可复用的系统，可随意替换任何模块，轻松添加额外模块
...
使用桥接模式将接口与实现解耦，区分 is-a 关系与 has-a 关系
...
使用表观模式，将子系统的复杂交互封装起来提供简易接口，从而避免用户直接与子系统交互而产生耦合。
...
使用装饰器装饰（组合）基础对象来动态地为其附加额外的职责。各个职责相互独立，若使用继承来扩展的话会产生很多类，代码过于冗余。
