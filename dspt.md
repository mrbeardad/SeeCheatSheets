...
- 面向对象：
  ...
  - 对象：Object 更准确的翻译应该叫物体，是对“世间万物”的抽象表示

  - 属性：即物体的特征，是物体的内部状态在具体环境状态下的外在表现，$P=f_{prop}(O, E)$

  - 方法：即物体的行为，是物体或环境进行状态转移的过程，$(O_{next}, E_{next})=f_{method}(I, O_{cur}, E_{cur})$

  > 以下是面向对象编程语言提供的基本机制

  - 封装：将一个物体的状态、特征、行为都放到一个类中，并隐藏不必要的细节；

  - 多态：将对其他物体的依赖描述为抽象接口，忽略不必要的细节；

  - 继承：获取其他物体特征和行为，再根据需要修改细节，目的是为了复用已有代码而降低开发成本。

- 设计模式：设计方案来解决复杂的问题或实现复杂目标，而方案的本质就是逻辑地组织进行若干状态转移以达到最终状态。

  1. 将复杂的逻辑抽离出去，
  2. 将通用的逻辑抽离出去；
  3. 将易变的逻辑抽离出去；

  - 如何选择封装成函数还是类方法？无需状态则函数，需要状态则类方法。
  - 抽离出去的逻辑封装成什么？函数参数，函数，类，方法。
  - 如何组织各个类？见下图，耦合度由高到低。
