...
9. 发布阶段：将pre_release|pre_release_gray并入release_a|release_b并部署正式线上发布环境（AB环境不固定是否为灰度环境）
10. 完全灰度：将灰度分支并入master


# 设计经验
* 代码分层：
    * controller负责**解析报文**并**响应前端**，保证逻辑简单且流程直接
    * bussiness负责**封装**复杂逻辑的业务代码
    * model负责**创建数据库**和读写数据库接口与写入数据规范

* 接口类型：
    * 对于前端接口的状态字段，需要使用可读性更好的字符类型而非数字类型
    * 对于数据库的存储类型，取决与效率与性能的折中，若存储字符串会导致性能瓶颈则改用整数
