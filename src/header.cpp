...
public:
    ...
    std::wstring operator()()
    {
        ...
        return L"\e[48;2;"
            + std::to_wstring(rgb_[0]) + L";"
            + std::to_wstring(rgb_[1]) + L";"
            + std::to_wstring(rgb_[2]) + L"m\e[38;2;"
            + std::to_wstring(0xff - rgb_[0]) + L";"
...
std::string& HeaderBlk::highlight(std::string& text)
{
    ...
    int nHyphen{(mediator_.getTtyCol()
    ...
    if ( text.starts_with("# FILE: ") ) {
        ...
        auto wTmpText = myconv.from_bytes(hyphen + ' ' + hashs + ' ' + normal + ' ' + hashs + ' ' + hyphen);
        std::wstring wText{};
        Color rgbColor{};
