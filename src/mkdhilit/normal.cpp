...
NormalBlk::NormalBlk() : MkdBlock{"normal"} {}
...
std::string& NormalBlk::highlight(std::string& text,
    ...
    for (size_t idx{}, end{origText.size()}; idx != end;) {
        switch (origText.at(idx)) {
            case '*':
                if (emphaStack.back() != Empha::CODE &&
                    emphaStack.back() != Empha::CODE2 &&
                    emphaStack.back() != Empha::CMT &&
                    (emphaStack.back() < Empha::ITL ||
                     emphaStack.back() > Empha::IBOLD)) {
            ...
            case '~':
                if (emphaStack.back() != Empha::CODE &&
                    emphaStack.back() != Empha::CODE2 &&
                    emphaStack.back() != Empha::CMT &&
                    emphaStack.back() != Empha::DEL && at(idx + 1) == '~') {
                    emphaStack.emplace_back(Empha::DEL);
                    colorStack.emplace_back(
                        colors[static_cast<size_t>(Empha::DEL)]);
                    text += colorStack.back();
                    idx += 2;
                } else if (emphaStack.back() == Empha::DEL &&
                           at(idx + 1) == '~') {
                    emphaStack.pop_back();
                    colorStack.pop_back();
                    text += colors.front() + colorStack.back();
            ...
            case '<':
                if (emphaStack.back() != Empha::CODE &&
                    ...
                    colorStack.emplace_back(
                        colors[static_cast<size_t>(Empha::UND)]);
                    text += colorStack.back();
                    idx += 3;
                } else if (emphaStack.back() == Empha::UND &&
                           substr(idx + 1, 3) == "/u>") {
                    emphaStack.pop_back();
                    colorStack.pop_back();
                    text += colors.front() + colorStack.back();
                    ...
                    colorStack.emplace_back(
                        colors[static_cast<size_t>(Empha::KBD)]);
                    text += colorStack.back();
                    idx += 5;
                } else if (emphaStack.back() == Empha::KBD &&
                           substr(idx + 1, 5) == "/kbd>") {
                    emphaStack.pop_back();
                    colorStack.pop_back();
                    text += colors.front() + colorStack.back();
                    ...
                    colorStack.emplace_back(
                        colors[static_cast<size_t>(Empha::CMT)]);
                    text += colorStack.back() + "<!--";
                    idx += 4;
                } else {
                    goto DEF;
            ...
            case '[':
                if (size_t pos{};
                    emphaStack.back() != Empha::CODE &&
                    emphaStack.back() != Empha::CODE2 &&
                    emphaStack.back() != Empha::CMT &&
                    emphaStack.back() != Empha::LINK &&
                    emphaStack.back() != Empha::LINK2 &&
                    ...
                    if (at(idx - 1) == '!') {
                        text.insert(text.size() - 1,
                                    colors[static_cast<size_t>(Empha::LINK)]);
                    } else {
            ...
            case ']':
                if (emphaStack.back() == Empha::LINK && at(idx + 1) == '(') {
                    emphaStack.back() =
                        Empha::LINK2;  // BUG: 截断LINK区间中未完成empha
                    colorStack.emplace_back(
                        colors[static_cast<size_t>(Empha::LINK2)]);
                    text += colors[static_cast<size_t>(Empha::LINK)];
                    text += "](";
                    text += colors.front() + colorStack.back();
                    idx += 2;
            ...
            case '`':
                if (emphaStack.back() != Empha::CODE &&
                    ...
                    emphaStack.back() != Empha::CMT) {
                    auto curEmpha =
                        at(idx + 1) == '`' ? Empha::CODE2 : Empha::CODE;
                    emphaStack.emplace_back(curEmpha);
                    colorStack.emplace_back(
                        colors.at(static_cast<size_t>(curEmpha)));
                    text += colorStack.back();
                    idx += static_cast<size_t>(curEmpha) -
                           static_cast<size_t>(Empha::CODE) + 1;
                } else if (emphaStack.back() == Empha::CODE ||
                           emphaStack.back() == Empha::CODE2 &&
                               at(idx + 1) == '`') {
                    idx += static_cast<size_t>(emphaStack.back()) -
