...
CodeBlk::CodeBlk() : mkdhilit::MkdBlock{"code"}, matchedEnd_{false} {}
...
std::string& CodeBlk::highlight(std::string& text) {
    ...
    // 同时去掉形如```cpp头部与形如```尾部
    std::stringstream sstrm{text.substr(
        type.size(), text.find_last_not_of("`\n ") + 2 - type.size())};
    text.clear();
    ...
    for (int cnt{}; cnt < nHyphen; ++cnt)
        hyphen += "─";
    text = "\033[38;2;255;165;0m┌" + hyphen + " \033[32m" + type +
           "\033[38;2;255;165;0m " + hyphen + "┐\033[m\n";
    ...
    for (std::string oneline{}; std::getline(hilitStrm, oneline);) {
        oneline =
            std::regex_replace(oneline, "\033\\[01;30m"_rgx, "\033[01;34m");
        text += "\033[38;2;255;165;0m│\033[m " + oneline +
                std::string(maxCol - nCols.at(idx++) - 4, ' ') +
                " \033[38;2;255;165;0m│\033[m\n";
    }

    text += "\033[38;2;255;165;0m└";
    for (int cnt{}; cnt < maxCol - 2; ++cnt)
        text += "─";
    text += "┘\033[m\n";
