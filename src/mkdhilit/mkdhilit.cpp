...
// MkdHighlight
/*************************************************************************************************/
MkdHighlight::MkdHighlight() : winsize_{}, blocks_{} {
        ...
        for (auto iter = blocks_.begin(); iter != blocks_.end(); ++iter) {
            if (iter->second->matchBegin(oneline)) {
                ...
                if (std::getenv("OUTPUT_DEBUG_LOG") != nullptr)
                    std::cerr << "\e[32mmatch:\e[m " << iter->first << ":"
                              << oneline;
                break;
            }
        }
        if (curBlk == nullptr)
            throw std::logic_error{"this line didn't match ans block: " +
                                   oneline};
...
std::tuple<std::vector<fs::path>, std::vector<std::string>, bool> parse_cmdline(
    ...
    auto print_help_then_throw = []() {
        std::cout
            << "\033[32mDescription:\033[m\n"
               "    \033[33msee\033[m will find cheat-sheets among "
               "~/.cheat/*.md.\n"
               "Yeah! They are markdown files. Each entry starts with "
               "'\033[34m<!-- entry begin: keywords... -->\033[m',\n"
               "and ends with '\033[34m<!-- entry end -->\033[m'. "
               "\033[33msee\033[m will search apposite entries and print "
               "them.\n"
               ...
               "        * echo\n"
               "            * -n        "
               "：不自动加入换行符（zsh会将无换行结尾的输出的尾部标记`反显的%`"
               "）\n"
               "            * -e        ：启用转义语义（zsh自动开启）\n"
               "        <!-- entry end -->\n"
               "\033[32mUsage:\033[m\n"
               ...
               "\n"
               "    You can redirect stdin to a file(not a tty) to use only "
               "Markdown Syntax Highlight.\n"
               "    see </path/to/file"
               "\033[32mOptions:\033[m\n"
               "    \033[36m-h\033[m                : Display this help "
               "information\n"
               "    \033[36m-f\033[m <file-prefix>  : Specify file in "
               "~/.cheat/ whoes file name prefix match <file-prefix> to "
               "search\n"
    ...
    // 处理参数无效的情况
    if (keys.empty()) {
        std::cerr << "\e[31m`see` Error:\e[0m Need entry-regex-keys"
                  << std::endl;
        print_help_then_throw();
    }
    ...
    for (const auto& thisDirEntry : fs::directory_iterator{cheatDir}) {
        if (!fs::is_regular_file(thisDirEntry) ||
            thisDirEntry.path().extension() != ".md")
            continue;
        ...
        if (filePrefix.empty()) {
            ...
            for (auto& thisFilePrefix : filePrefix) {
                // 确认文件名前缀是否与用户指定匹配
                if (curFileName.find(thisFilePrefix) == 0) {
                    files.emplace_back(thisDirEntry.path());
                    break;
                }
            }
...
std::string search_entries(const std::vector<fs::path>& files,
    ...
    for (auto& thisFile : files) {
        ...
        for (std::string oneline{}; std::getline(fstrm, oneline);) {
            ...
            if (!inEntry && oneline.find("<!-- entry begin:") == 0 &&
                oneline.rfind("-->") == oneline.length() - 2) {
                ssize_t prefix{sizeof("<!-- entry begin:") - 1},
                    suffix{sizeof("-->") - 1};

                // 有一个key不匹配则略过
                bool isMatch{true};
                for (auto& thisKey : keys) {
                    if (!std::regex_search(oneline.begin() + prefix,
                                           oneline.end() - suffix,
                                           std::regex{thisKey})) {
                        isMatch = false;
                        break;
