...
TableBlk::TableBlk() : MkdBlock{"table"} {}
...
std::string& TableBlk::highlight(std::string& text) {
    ...
    if (std::regex_search(line2nd, g_Table2nd)) {
        ...
        for (auto pos = line2nd.begin() + 1, end = line2nd.end(); pos != end;
             ++pos) {
            if (*pos == '|') {
                newLine1st += "┬";
                newLine3rd += "┼";
        ...
        std::string normal{};
        for (auto pos = newLine2nd.begin() + 1, end = newLine2nd.end();
             pos != end; ++pos) {
            if (*pos != '|') {
                normal += *pos;
            } else {
                text += MkdHighlight::Instance().getBlock("normal").highlight(
                            normal) +
                        "│";
                normal.clear();
            }
        ...
        for (int cnt{}; std::getline(sstrm, line2nd); ++cnt) {
            ...
            text +=
                dynamic_cast<NormalBlk&>(normalBlk).highlight(
                    line2nd, cnt % 2 ? "\033[38;5;240m\033[48;2;94;175;220m"
