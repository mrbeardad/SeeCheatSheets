...
* 答：可能就是这两年来我靠自己一步步走到今天吧。
    我从大一下期决定自学计算机的，当时因为信念还不坚定放弃了转专业的机会，然后就错过了。
...
* 答：缺点就是注意力不集中，有时候看一句话要来来回回看几遍，但其实心思根本没在上面，
    这句子明明都是中文，但就是怎么看都看不懂。
    有时候会想，刚刚学的知识点好像有点忘了，要不要再回去学一遍，但经常这只是在浪费时间。
    不过有时候也得益于这种发散思维，会把一些知识点联想起来，建立思维导图。
...
* 二叉搜索树
    ...
    * 特点：树的深度平均为O(log(N))，使得搜索操作的时间复杂度平均为O(log(N))，加快搜索速度的同时可以对数据进行排序
...
* 散列表
    ...
    * 哈希冲突：
        * 分离链表法：
            * 特点：将哈希到同一个下标的元素保存到一个链表中，相同元素放到一起，新插入的放前面，
...
* 行记录
    * 变长字段长度列表:NULL值列表:头部信息:主键列:事务ID:回滚指针:其他字段列
        > 头部信息包含有：下条行记录指针、删除位等等  
        > 若表中没有NOT NULL UNIQUE列，则自动生成隐藏的主键列  
        > 事务ID为最后一次修改该行的事务ID  
        > 回滚指针指向Undo Log
    * 行溢出处理：行记录数据列最长为65535字节（不计算记录头信息、隐藏列、TEXT/BLOB族类型列）
        而数据页一般为16K且规定一页至少放两行记录，一般将过长的列存储与外部的溢出页中，行记录中值存储指针

* 页结构
    * 头部信息:用户记录空间:空闲空间:页面目录:尾部校验
        > 头部信息包括：上/下个页面指针（页面即是B+树节点）、空闲空间地址、垃圾回收链表等等  
        > 用户记录：按索引列大小排列成有序链表  
