...
### 服务架构
#### 全局负载均衡
* 实现在广域网上不同地域的服务器间的流量调配，让用户就近接入，一般基于DNS

#### 网关路由
* IP转发
    * 四层转发：根据(VIP+VPORT+PROTOCOL)识别业务
    * 七层转发：根据http协议报文中的域名识别业务

* 负载均衡：
    * 根据权重选择一个Server并转发数据到其上
    * 自动剔除和恢复server
...
![k8s](images/components-of-kubernetes.svg)

> 为了简单起见，设置脚本通常会在同一个计算机上启动所有控制平面组件， 并且不会在此计算机上运行用户容器。
