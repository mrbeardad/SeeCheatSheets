...
- Go
  - 程序入口：main包中main函数
  - 依赖单元：一个目录（不包括子目录）
  - 依赖导入：`import "module.github.com/path/to/package"`，初始化：所有init函数
...
| Method             | `CapWords`             |
| Constants          | `kCapWords`            |
| Static Variables   | `kCapWords`            | 注意不会调用静态生命周期对象的析构函数，使用ref-return代替global变量 |
| Local Variables    | `lower_with_under`     |
| struct Data Member | `lower_with_under`     | static同                                                             |
| class Data Member  | `lower_with_under_`    | static同                                                             |
