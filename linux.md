...
### 中断
&emsp;操作系统通过CPU中断来获取CPU控制权并进入内核态。
想象单核CPU上同时只能运行一个程序，即同时只能有一个控制流。
若没有操作系统进行调用，则用户只能同步的运行程序并等待其结束后运行下一个程序。
但是只有一个控制流，那操作系统又怎么知道何时，以及如何从用户程序的控制流中收回CPU控制权呢。

&emsp;这就需要CPU中断机制——CPU接收引脚上的电信号，而该电信号则意味着一个外部事件的发生。
接下来CPU会自动切换控制流，寻找操作系统提供的异常处理表，并将控制流移交给对应的异常处理程序。
这时CPU控制权从用户程序切换到操作系统。借助这些中断信号，于是操作系统就有随时掌控计算机资源的能力。

&emsp;每过一段相当短的一段时间时（通常几毫秒），一个定时器向CPU的一个引脚上发送中断信号。
CPU切换控制流并搜索操作系统的异常处理表，然后控制权移交给操作系统调度程序。
内核调度程序的调度算法会根据一些其他与进程相关的信息来选择下一个用户进程的控制流。
记住内核调度是以 **线程** 为基本单位。

&emsp;一般而言，中断对于用户态进程是不可见的，
它不会改变操作系统提供给我们的抽象的进程模型：
* 进程占用连续的内存
* 进程拥有连续的控制流
...
| SIGTTIN  | 后台进程组中进程试图读取终端，发射该信号给该后台进程组，若进程阻塞或忽略该信号则不发送，但会使本次read调用失败                          |
