...
## 动态绑定
* 接口值底层数据结构包含两个指针，一个指向类型信息，一个指向动态值
```go
...
## 映射
* 映射值底层即一个指向底层哈希表栈结构的指针，意味着即使底层哈希表rehash时映射值仍有效指向rehash后的哈希表
* 相对应的，切片值本身就是底层动态数组的栈结构，意味着内存重分配时切片值仍指向旧的动态数组
```go
