...
## 命名空间
* 命名空间可看作一种C++代码逻辑的组织方式
* 头文件中的符号都应放在命名空间中，实现文件代码最好也放入同一命名空间
* 头文件中的符号，要么只提供接口声明，要么提供定义必须声明inline
```cpp
// 命名空间：第一次声明为定义，以后为打开
namespace ns
{
...
// 全局命名空间
::member // 明确限定引用全局符号，但位与内联命名空间中同名符号冲突
...
// 模板类型别名
template<typename T>
using Alias = TempType<T, int>  // 别名类型涉及到模板
...
* 分支
```cpp
if constexpr ( init; condition ) { // `init;`部分可忽略
    /* ... */
} else if ( init; constexpr_bool ) { // `init;`部分可忽略
    /* ... */
} else {
    /* ... */
}
